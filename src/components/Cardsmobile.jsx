...
export function CardsMobile() {
  ...
  return <MobileCarousel urls={urls} />;
}

function MobileCarousel({ urls }) {
  const anchor = useRef();
  const meshA = useRef();
  const meshB = useRef();
  ...
  const anim = useRef({ playing: false, t: 0, dir: 0, dur: 0.3 });

  // --- PRIVATE loader so useProgress doesn’t fire ---
  const loaderRef = useRef(null);
  if (!loaderRef.current) {
    const privateManager = new THREE.LoadingManager(); // NOT DefaultLoadingManager
  ...
  const setMaps = (trio, tex) => {
    ...
    back.map = tex;
    back.needsUpdate = true; // upright art
  };
  ...
  const go = useCallback(
    async (dir) => {
      ...
      anim.current = { playing: true, t: 0, dir, dur: 0.3 };

      // silent prefetch of neighbors
  ...
  // Loop: float on anchor, slide on children (refs only)
  useFrame((state, dt) => {
    ...
    if (a) {
      ...
      a.scale.x = THREE.MathUtils.lerp(a.scale.x, BASE_SCALE * s, 1 - Math.exp(-K * dt));
      a.scale.y = THREE.MathUtils.lerp(a.scale.y, BASE_SCALE * s, 1 - Math.exp(-K * dt));
      a.scale.z = 1;
    }
    ...
    if (anim.current.playing && inMesh && outMesh && inMats && outMats) {
      ...
      if (anim.current.t >= 1) {
        ...
        indexRef.current = (indexRef.current + dir + urls.length) % urls.length;
  ...
  return (
    <group ref={anchor}>
      ...
      <mesh ref={meshB} position={[SLIDE_X, 0, 0]}>
        <boxGeometry args={[CARD_W, CARD_H, CARD_D]} />
      </mesh>

      {/* arrows: exactly your sizes/positions */}
      <Html>
